...
def earth_to_body_frame(ii, jj, kk):
    # C^b_n
    R = [
        [C(kk) * C(jj), C(kk) * S(jj) * S(ii) - S(kk) * C(ii), C(kk) * S(jj) * C(ii) + S(kk) * S(ii)],
...
class PhysicsSim:
    def __init__(self, init_pose=None, init_velocities=None, init_angle_velocities=None, runtime=5.0):
        ...
        width, length, height = 0.51, 0.51, 0.235
        ...
        I_x = 1 / 12.0 * self.mass * (height ** 2 + width ** 2)
        I_y = 1 / 12.0 * self.mass * (height ** 2 + length ** 2)  # 0.0112 was a measured value
        I_z = 1 / 12.0 * self.mass * (width ** 2 + length ** 2)
    ...
    def reset(self):
        ...
        self.angular_v = (
            np.array([0.0, 0.0, 0.0]) if self.init_angle_velocities is None else np.copy(self.init_angle_velocities)
        )
        self.linear_accel = np.array([0.0, 0.0, 0.0])
        self.angular_accels = np.array([0.0, 0.0, 0.0])
        self.prop_wind_speed = np.array([0.0, 0.0, 0.0, 0.0])
        self.done = False
    ...
    def calc_prop_wind_speed(self):
        ...
        s_0 = np.array([0.0, 0.0, theta_dot * self.l_to_rotor])
        s_1 = -s_0
        s_2 = np.array([0.0, 0.0, phi_dot * self.l_to_rotor])
        s_3 = -s_2
    ...
    def get_propeler_thrust(self, rotor_speeds):
        ...
        for prop_number in range(4):
            ...
            C_T = max(0.12 - 0.07 * max(0, J) - 0.1 * max(0, J) ** 2, 0)
    ...
    def next_timestep(self, rotor_speeds):
        ...
        angles = (
            self.pose[3:] + self.angular_v * self.dt + 0.5 * self.angular_accels * self.angular_accels * self.dt ** 2
        )
